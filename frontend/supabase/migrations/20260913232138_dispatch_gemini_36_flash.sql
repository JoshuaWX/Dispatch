-- Gemini 2.5 Flash billing history remains valid audit data. New usage records
-- are constrained by the settlement function below to Gemini 3.6 Flash only.
...
alter table public.dispatch_ai_usage
  ...
  check (model in ('gemini-2.5-flash', 'gemini-3.6-flash'));
...
begin
  ...
  values (p_reservation_id, reservation.run_id, 'gemini-3.6-flash', p_input_tokens, p_output_tokens, p_actual_usd);
end;
$$;
