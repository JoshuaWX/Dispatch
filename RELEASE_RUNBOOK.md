...
- 2026-09-24: Staging Preview's `SUPABASE_SERVICE_ROLE_KEY` is Vercel-sensitive and cannot be read back by `vercel env pull`. A direct transfer failed closed. A one-time, bearer-protected Preview route then used Vercel's runtime service key to copy the rotated automation bypass into staging Vault, returning HTTP 200. The temporary database RPC was dropped and confirmed absent, and the exact branch-scoped sensitive Vercel transfer variable was removed. The one-time route was removed from the final branch. Staging Cron is not yet installed and the 48-hour soak has not started.
- 2026-09-24/25: Staging Cron is installed as `dispatch-staging-every-two-hours` at minute 17 of each even UTC hour. Its first request through the exact Vault-backed scheduler/bypass path returned HTTP 200 and a `publishing_paused` result. After both staging controls were enabled, autonomous runs used only `gemini-3.1-flash-lite`; three rights-complete, verification-passed official announcements were published and unsafe drafts stayed private. Staging usage is `$0.024520` against its `$0.19` cap. Evidence or verification rejections remain excluded for 72 hours; a one-off malformed Gemini response gets one fresh, budget-reserved retry and one later scheduled evaluation before Dispatch moves to a newer eligible story.
- 2026-09-25: PR #7 head `3228eb6922fa5d0e6276b8a1379a1bf6e800cd3d` passed quality, secret scanning, database migration (including all 124 legacy records), and browser acceptance in GitHub Actions. Its Vercel Preview is ready at the same commit. The PR remains a draft until the staging soak completes; production publishing remains disabled.
