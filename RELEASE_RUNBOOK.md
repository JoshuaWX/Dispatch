...
- 2026-09-24/25: Staging Cron is installed as `dispatch-staging-every-two-hours` at minute 17 of each even UTC hour. Its first request through the exact Vault-backed scheduler/bypass path returned HTTP 200 and a `publishing_paused` result. After both staging controls were enabled, autonomous runs used only `gemini-3.1-flash-lite`; three rights-complete, verification-passed official announcements were published and unsafe drafts stayed private. Staging usage is `$0.024520` against its `$0.19` cap. A rejected topic is now excluded for 72 hours from scheduled selection, avoiding repeat Gemini spend while allowing an explicit operator retry.
